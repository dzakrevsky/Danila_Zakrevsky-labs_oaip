...
private:
    ...
    void dump0(Node<T, K> const * node, QString& vv, QString prefix = "", bool root = true, bool last = true) {
        vv+=  prefix + (root ? "" : (last ? "//-" : "|-")) + (node ? node->data + ", " + QString::number(node->key) : "") + '\n';
        if (!node || (!node->left && !node->right)){
            return;
        }
        std::vector<Node<T, K>*> v{node->left, node->right};
        for (size_t i = 0; i < v.size(); ++i)
            dump0(v[i], vv,prefix + (root ? "" : (last ? "  " : "| ")), false, i + 1 >= v.size());
    }
